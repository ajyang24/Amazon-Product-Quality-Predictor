##
# NOTE: THIS FILE WAS WHAT WE USED TO TRAIN OUR MODELS AND SEE HOW
# THEY PERFORMED. used K fold verifaction here and code might be cleaner
##
import pandas as pd
import pickle
import numpy as np
...
def use_model(read_product):
    ...
    def run_model(final_df):
        ...
        shaped_df_features = final_df[['0','6']]
        ...
        for i in range(k):
            ...
            y_pred = single_pred()
            print(y_pred.shape)
            final_df['awesomeness'] = pd.Series(y_pred.ravel(), name='temp')
            print(final_df)
